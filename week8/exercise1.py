...
def make_filler_text_dictionary():
    ...
    def func(x):
        return requests.get(url+str(x))
    return {k: [func(k).text for _ in range(3)] for k in range(3, 8)}
...
def random_filler_text(number_of_words=200):
    ...
    for _ in range(number_of_words):
        word_count = random.choice(range(3, 8))
        L.append(random.choice(d[word_count]))
    L[0] = L[0].capitalize()
    return ' '.join(L) + '.'
...
def fast_filler(number_of_words=200):
    ...
    """
    print(os.path.isfile("dict_racey.words"))
    if not os.path.isfile("dict_racey.words"):
        with open("dict_racey.words", 'w') as myfile:
            myfile.write(str(make_filler_text_dictionary()))
    else:
        with open("dict_racey.words", 'r') as myfile:
            d = {int(k): v for k, v in ast.literal_eval(myfile.read()).items()}
            L = []
            for _ in range(number_of_words):
                word_count = random.choice(range(3, 8))
                L.append(random.choice(d[word_count]))
